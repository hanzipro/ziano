...
def _build_vf(fam: FamilyConfig, archive: Path, work: Path, slices: list[Slice],
...
def _build_static(fam: FamilyConfig, archive: Path, work: Path, slices: list[Slice],
    ...
    for w in weights:
        font_path = extract_member(archive, w.member, work)
        for s in slices:
...
def build_family(family_id: str, *, roster_path: str, dest: str, version: str,
    ...
    return _build_static(fam, archive, work, slices,
                         dest=dest, version=version, license_text=license_text,
